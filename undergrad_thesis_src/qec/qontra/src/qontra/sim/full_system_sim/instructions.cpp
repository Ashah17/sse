...
FullSystemSimulator::read_next_instruction(const qes::Program<>& from, program_status_t& st) {
    ...
    if (instruction.has_annotation("timing_error") && !st.return_if_waiting_trials[0]) {
        inject_timing_error();
    }
    ...
    if (is_gate(instruction)) {
        fp_t lat = do_gate(instruction, st);
        ...
        create_event_or_obs(instruction, st);
    ...
    if (is_2q_gate(instruction) && !st.return_if_waiting_trials[0]) {
    ...
    // std::cout<<"here is w from part 2: " << w << std::endl;
}

fp_t
FullSystemSimulator::do_gate(const qes::Instruction<>& instruction, program_status_t& st, int64_t trial) {
    ...
    if (name == "measure") { 
        return do_measurement(instruction, st, trial);
    } else if (name == "h") {
    ...
    if (st.return_if_waiting_trials[0]) {
        return 0;
    }
...
fp_t
FullSystemSimulator::do_measurement(const qes::Instruction<>& instruction, program_status_t& st, int64_t trial) {
    // std::cout<<"making measure"<<std::endl;
    std::vector<uint64_t> qubits = get_qubits(instruction);
    ...
    for (size_t i = 0; i < qubits.size(); i++) {
        uint64_t q = qubits[i];
        if (st.return_if_waiting_trials[0]) {
            m1w0_array.push_back(0);
            m0w1_array.push_back(0);
        } else {
        ...
        if (is_recording_stim_instructions && trial < 0 && !st.return_if_waiting_trials[0]) {
    ...
    std::vector<int> measures = base_sim->M(qubits, m1w0_array, m0w1_array, meas_ctr+meas_offset, trial);

    //saving the current measures into this vector, pass it one more time
...
void
FullSystemSimulator::create_event_or_obs(const qes::Instruction<>& instruction, program_status_t& st) {
    std::string name = instruction.get_name();
    int64_t index = instruction.get<int64_t>(0);
    if (name == "event") index += event_offset;
    
    stim::simd_bits_range_ref<SIMD_WIDTH> w = name == "event" ? syndrome_table[index] : observable_table[index];
    ...
    for (size_t i = 1; i < instruction.get_number_of_operands(); i++) {
        ...
        // if ((w ^ base_sim->record_table[k]).popcnt() != 0) {
        //     count++;
        // }

        // if (w.popcnt() != base_sim->record_table[k].popcnt()) {
        //     std::cout<<"error"<<std::endl;
        // }

        // std::cout<<w.popcnt()<<std::endl;
        // std::cout<<base_sim->record_table[k].popcnt()<<std::endl;
        // std::cout<<"pass"<<std::endl;
    ...
    // if(syndrome_table[index].popcnt() != 0) {
    //     std::cout<<syndrome_table[index]<<std::endl;
    //     std::cout<<index<<std::endl;
    // }
    

    // stim::simd_bits_range_ref<SIMD_WIDTH> event_outcome = syndrome_table[index];
    // for (size_t i = 0; i < event_outcome.num_bits_padded(); i++) {
    //     bool bit_value = event_outcome[i];  // Accessing each bit.
    //     if (bit_value) {
    //         count++;
    //         // std::cout << "Bit " << i << ": " << bit_value << std::endl;
    //     }
    // }

    // std::cout << "Bits 1 "  << ": " << count << std::endl;

    // // if (w[0] && st.return_if_trials_waiting[0]) {
    ...
    // std::cout<<"here is w from part 1: " << w << std::endl;
}
