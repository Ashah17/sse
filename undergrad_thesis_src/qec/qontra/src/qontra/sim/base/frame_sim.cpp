...
std::vector<int> qontra::FrameSimulator::measures;
...
FrameSimulator::M(
        ...
        int64_t tr)
{
    //reset the measures variable everytime m is called
    measures.clear();
    size_t k = 0;
    for (uint64_t i : operands) {
        if (tr >= 0) {
        ...
        if (record >= 0) {
            if (tr >= 0) {
                ...
                if (record_table[record].popcnt() > 0) {
                    //non zero hw
                ...
                // std::cout<<"record num "<< record << "is " << record_table[record] << std::endl;
            }
            record++;
            k++;
        }
    ...
    return measures; //hype it passed thru
}
