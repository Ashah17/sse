...
public:
    ...
    std::vector<int>    M(std::vector<uint64_t>, std::vector<fp_t>, std::vector<fp_t>, int record=-1, int64_t tr=-1) override;
    void    R(std::vector<uint64_t>, int64_t tr=-1) override;

    static std::vector<int> measures; //make static for class
