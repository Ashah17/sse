...
pm::MatchingResult shatter_blossoms_for_all_detection_events_and_extract_obs_mask_and_weight(
    ...
    for (auto& i : detection_events) {
        ...
        if (mwpm.flooder.graph.nodes[i].region_that_arrived){
            // auto hype = mwpm.shatter_blossom_and_extract_matches(mwpm.flooder.graph.nodes[i].region_that_arrived_top);
            // std::cout<<hype.obs_mask<<std::endl;
    ...
    // std::ofstream outFile("decodedObservables.txt", std::ios::app);

    // outFile << "SHOT IS " << shot_num << "AND OBS IS " << res.obs_mask << std::endl;

    // outFile.close();
    

    // std::ofstream outFile("weightchecks.txt", std::ios::app);

    // outFile<< shot_num << "AND WEIGHT IS " << res.weight <<std::endl;
