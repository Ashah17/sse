...
static int total_obs_mask = 0;
static int obs_mask_set_when_boundary_is_null = 0; //counters
...
MatchingResult Mwpm::shatter_blossom_and_extract_matches(GraphFillRegion *region) {
    ...
    // std::ofstream outFile("decoding_frame_flip_analysis.txt", std::ios::app);

    // outFile<<"SHOT IS " << shot_num<<std::endl;

    // shot_num++;

    if(region->match.edge.obs_mask == 1) {
        total_obs_mask++;
        if (region->match.edge.loc_to == NULL) {
            obs_mask_set_when_boundary_is_null++;
        }
    }

    // outFile<<total_obs_mask<<std::endl;
    // outFile<<obs_mask_set_when_boundary_is_null<<std::endl;

    // std::cout<<region->match.edge.obs_mask<<std::endl;
