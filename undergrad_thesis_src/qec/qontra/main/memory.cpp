...
int main(int argc, char* argv[]) {
    ...
    //shud try to pass in another param here with the round nums that r skipped
    //WHAT DISTINGUISHES A SKIPPED ROUND WITHIN THE PROGRAM RN
    
    DetailedStimCircuit circuit = make_circuit(qes_file, p);
    // PyMatching dec(circuit);
    
    
    //trying the sliding window
    SLWDecoder dec(circuit, 8, 1); //d round is 3 w
